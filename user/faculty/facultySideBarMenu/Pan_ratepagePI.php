...
<?php
   ...
   //$sql4 = "SELECT user_id,name,a.group_id,group_name,phase_id,phase_name,schedule_id,group_sc,phase_sc,time_start,time_end,venue,thesis_id,b.group_id,thesis_title from group_tbl a INNER JOIN users_tbl on user_id = adviser INNER JOIN schedules_tbl on a.group_id = group_sc INNER JOIN phases_tbl on phase_id = phase_sc INNER JOIN thesis_tbl b on a.group_id = b.group_id where phase_id = 1 ";
    ...
    //$sql4 = "SELECT thesis_phase_id,a.thesis_id,a.phase_id,thesis_phase_status,b.thesis_id,b.group_id,thesis_title,c.phase_id,phase_name,d.group_id,group_name,adviser,user_id,name from thesis_phase_tbl a INNER JOIN thesis_tbl b on a.thesis_id = b.thesis_id INNER JOIN phases_tbl c on a.phase_id = c.phase_id INNER JOIN group_tbl d on b.group_id = d.group_id INNER JOIN users_tbl on user_id = adviser where a.phase_id = 3 "; //old join

    //$sql4 = "SELECT thesis_phase_id,a.thesis_id,a.phase_id,thesis_phase_status,b.thesis_id,b.group_id,thesis_title,c.phase_id,phase_name,d.group_id,group_name,adviser,user_id,name,e.group_id,panelist_id from thesis_phase_tbl a INNER JOIN thesis_tbl b on a.thesis_id = b.thesis_id INNER JOIN phases_tbl c on a.phase_id = c.phase_id INNER JOIN group_tbl d on b.group_id = d.group_id INNER JOIN users_tbl on user_id = adviser INNER JOIN thesis_panels_tbl e on b.group_id = e.group_id where a.phase_id = 3 ";//dependent on thesis_phase_tbl

    $sql4 = "SELECT user_id,name,a.group_id,group_name,phase_id,phase_name,schedule_id,group_sc,phase_sc,time_start,time_end,venue,thesis_id,b.group_id,thesis_title,c.group_id,panelist_id from group_tbl a INNER JOIN users_tbl on user_id = adviser INNER JOIN schedules_tbl on a.group_id = group_sc INNER JOIN phases_tbl on phase_id = phase_sc INNER JOIN thesis_tbl b on a.group_id = b.group_id INNER JOIN thesis_panels_tbl c on a.group_id = c.group_id where phase_sc = 3";//dependent on schedules_tbl
...
<td>
<!--<a href="Pan_ratepagePI_rate.php?panelrate=<?php // echo htmlentities( $row4->group_id); ?>"> Rate </a>-->
<a href="Pan_ratepagePI_rate.php?panelrate=<?php echo htmlentities( $row4->thesis_id); ?>"> Rate </a>
<a>| |</a>
<a href="Pan_ratepagePI_OE.php?viewmembers=<?php echo htmlentities( $row4->group_id); ?>"> View Members </a>
</td> 
                    </tr>
                  <?php } //} ?>
